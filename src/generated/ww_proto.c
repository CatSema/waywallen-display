...
/* clang-format off */
/* Source: protocol/waywallen_display_v9.xml */
...
static int w_transition_kind(ww_buf_t *b, waywallen_transition_kind_t v) {
    return w_u32(b, (uint32_t)v);
}

static int rd_transition_kind(ww_rd_t *r, waywallen_transition_kind_t *out) {
    uint32_t value;
    int rc = rd_u32(r, &value);
    if (rc) return rc;
    switch (value) {
    case 0: *out = WAYWALLEN_TRANSITION_KIND_NONE; return WW_OK;
    case 1: *out = WAYWALLEN_TRANSITION_KIND_FADE; return WW_OK;
    case 2: *out = WAYWALLEN_TRANSITION_KIND_WIPE; return WW_OK;
    case 3: *out = WAYWALLEN_TRANSITION_KIND_GROW; return WW_OK;
    default: return WW_ERR_BAD_ENUM;
    }
}
...
static int w_transition_config(ww_buf_t *b, const waywallen_transition_config_t *v) {
    int rc;
    if ((rc = w_transition_kind(b, v->kind))) return rc;
    if ((rc = w_u32(b, v->duration_ms))) return rc;
    if ((rc = w_u32(b, v->angle))) return rc;
    if ((rc = w_f32(b, v->origin_x))) return rc;
    if ((rc = w_f32(b, v->origin_y))) return rc;
    return WW_OK;
}

static int rd_transition_config(ww_rd_t *r, waywallen_transition_config_t *v) {
    int rc;
    if ((rc = rd_transition_kind(r, &v->kind))) return rc;
    if ((rc = rd_u32(r, &v->duration_ms))) return rc;
    if ((rc = rd_u32(r, &v->angle))) return rc;
    if ((rc = rd_f32(r, &v->origin_x))) return rc;
    if ((rc = rd_f32(r, &v->origin_y))) return rc;
    return WW_OK;
}

static void free_transition_config(waywallen_transition_config_t *v) {
    (void)v;
}

void waywallen_transition_config_free(waywallen_transition_config_t *value) {
    if (!value) return;
    free_transition_config(value);
    memset(value, 0, sizeof(*value));
}
...
static int w_presentation_config(ww_buf_t *b, const waywallen_presentation_config_t *v) {
    ...
    if ((rc = w_transition_config(b, &v->transition))) return rc;
    return WW_OK;
}
